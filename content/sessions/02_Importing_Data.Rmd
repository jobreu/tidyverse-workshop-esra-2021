...
gpc <- readr::read_csv("./data/ZA5667_v1-0-0_Stata14_synthetic-data.csv")
```
...
gpc <- readxl::read_xlsx("./data/ZA5667_v1-0-0_Stata14_synthetic-data.xlsx")
```
...
gpc <- 
  haven::read_stata("./data/ZA5667_v1-0-0_Stata14_synthetic-data.dta")
```
...
```{r readr_example_col_change, echo = TRUE, eval = FALSE}
titanic <-
  read_csv(
...
```{r readr_example_col_change_display, echo = TRUE, eval = FALSE}
titanic
```
...
```{r readr_example_col_changeD, echo = TRUE, eval = FALSE}
titanic <-
  read_csv(
    "../../data/titanic/titanic.csv",
    col_types = cols(
...
```{r readr_example_col_changeD_display, echo = TRUE, eval = FALSE}
titanic
```
...
```{r readr_example_col_change_after, echo = TRUE, eval = FALSE}
...
```{r readr_example_col_changeD_after, echo = TRUE, eval = FALSE}
titanic <-
  type_convert( #<<
    titanic,
    col_types = cols(
...
```{r, out.width = "50%"}
include_graphics(normalizePath("./content/img/export_data.png"))
```
...
readr::write_csv(gpc, "gpc_own.csv")
```

```{r export_stata, echo = TRUE}
haven::write_dta(gpc, "gpc_own.dta")
```
