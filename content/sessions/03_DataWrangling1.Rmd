...
Another particularly useful selection helper is `where()`. You can, e.g., use `where()` to select only a specific type of variables, such as `select(where(is.numeric))`. As the synthetic dataset we use in our course only includes numeric variables, however, we will instead use `where()` to select only dichotomous variables (that have a *min* of 0 and a *max* of 1).
...
# [Exercise](https://jobreu.github.io/tidyverse-workshop-esra-2021/exercises/DataWrangling1_question.html) time `r ji("weight_lifting_woman")``r ji("muscle")``r ji("running_man")``r ji("biking_man")`
